...
function App() {
    ...
    if (index !== 2) {
      window.open(
        ...
        index === 0
          ...
          : `toolbar=no, menubar=no, location=yes,height=${
              window.innerHeight / 4
            },width=${window.innerWidth / 4},scrollbars=no,status=yes`
      );
    } else {
      window.close();
    }
  ...
  const updateNewPosition = ({ x, y }) => {
    bc.postMessage({
      ...
      value: { x, y, windowX: window.screenX, windowY: window.screenY },
    });
    setPosition({ x, y });
  };
    ...
    bc.onmessage = (event) => {
      switch (event.data.type) {
        ...
        case "update_position": // sync image's position accross window
          ...
          setPosition({
            x: absoluteX - window.screenX,
            y: absoluteY - window.screenY,
          });
          break;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(function () {
      if (oldX !== window.screenX || oldY !== window.screenY) {
        updateNewPosition({
          x: position.x - window.screenX + oldX,
          y: position.y - window.screenY + oldY,
        });
      }
      oldX = window.screenX;
      oldY = window.screenY;
    ...
    <>
      {loading && index !== 0 && (
        <div class="fixed top-0 left-0 right-0 bottom-0 w-full h-screen z-50 overflow-hidden bg-gray-700 opacity-75 flex flex-col items-center justify-center">
          ...
          <p class="w-1/3 text-center text-white">
            Đừng lóng, chờ 1 xíu là tải xong...
          </p>
        </div>
      )}
      <div className="App w-[100vw] h-[100vh] bg-gray-50 dark:bg-gray-800 pt-2">
        <div className="container mx-auto text-center pt-2 px-1">
          ...
          {index === 0 ? (
            <>
              ...
              <a
                target="_blank"
                href="https://github.com/trucka312/hentaiz"
            ...
            <div className="flex flex-col gap-y-2 mb-2">
              ...
              >
                {images.map((image, index) => (
                  <option value={index}>{image.title}</option>
                ))}
              </select>
            </div>
          )}
        ...
        {index !== 0 && (
          ...
          >
            <img
              className="cursor-move max-w-[900px]"
